...
const ProfitUser = () => {
    ...
    const [availablebalance, setValue] = useState("");
    ...
    const handleSearch = async (e) =>{
        e.preventDefault();
        const queryVar = query(collection(db, "user"),
    ...
    const userId = userData.userId
        ...
        const updateProfit = doc(db, "user", userId)
        await updateDoc(updateProfit, {
            availablebalance
        })
        alert('has been updated')
        
        
        // const res = await axios.put(`/admin/updateuserprofit/${userData._id}`, 
        // {availablebalance})
        // res.status === 200 && alert(`${userData.username} has been update`)
    }
  ...
  return (
    ...
    <Wrapper>
        ...
        name='phonenumber'
        value={`Phone Number: 0${userData.phonenumber}`}
        />
        </Wrapper>
    <Wrapper>
        ...
        value={`Available Balance: ${userData.availablebalance}`}
    ...
    <Wrapper>
        ...
        value={`Account Name: ${userData.accname}`}
        name='accname'
    />
        <Input readOnly placeholder='Account Number'
        value={`Account Number: ${userData.accnumber}`}
        name='accnumber'       
        />
        <Input readOnly placeholder='Bank Name'
        value={`Bank Name: ${userData.bankname}`}
        name='bankname'
        />  
    </Wrapper>
    <Wrapper>
   ...
   <BtnContainer>
   <Input readOnly placeholder='Plan Name'
        value={`Plan Name: ${userData.activatedPlan}`}
        name='bankname'
        />
        <Input readOnly placeholder='Bank Name'
        value={`Daily Profit: ${userData.userdailyprofit}`}
        name='bankname'
        />      

<Input type='number' placeholder='Today Profit'
